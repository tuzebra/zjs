// include plugin paste for editor
;zjs.require('editor', function(){
	...
	(function() {
		var each = tinymce.each,
			defs = {
				...
				paste_convert_table : false,
				paste_convert_div : false,
				paste_convert_iframe : false,
				paste_convert_input : true,
		...
		tinymce.create('tinymce.plugins.PastePlugin', {
			...
			_preProcess : function(pl, o) {
				...
				if (getParam(ed, "paste_convert_table")) {
					process([
						[/<\/?table[^>]*>/gi, ""],
						[/<\/?tbody[^>]*>/gi, ""],
						[/<\/?thead[^>]*>/gi, ""],
				...
				if (getParam(ed, "paste_convert_iframe")) {
					process([
						[/<\/?iframe[^>]*>/gi, ""]
					]);
				};
				
				if (getParam(ed, "paste_convert_input")){
					process([
						[/<input[^>]*>[^<]*<\/input>/gi, ""],
						[/<input[^>]*>/gi, ""],
				...
				if (getParam(ed, "paste_convert_div")) {
					process([
						[/<div[^>]*>/gi, "<p>"],
						[/<\/div[^>]*>/gi, "</p>"]
					]);
	...
	zjs.extend(zjs.moduleEditorPlugins, {
		paste: function(option, editorOption){
			
			// neu nhu option cua plugin bat len thi moi extend them option cho editor
			if('pasteplaintext' in option && !option.pasteplaintext){
			...
			return zjs.extend(editorOption, {
				...
				paste_convert_table : true,
 				paste_convert_div : true,
  				paste_convert_iframe : true,
  				paste_convert_input : true,
				paste_dialog_width : "450",
				paste_dialog_height : "400",
				paste_max_consecutive_linebreaks: 2,
