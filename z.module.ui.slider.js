...
zjs.require('ui', function () {
	...
	var sliderclass = 'zui-slider',
		...
		sliderwrapclass = 'zui-slider-wrapper',
	...
	var makeUiSlider = function (element, useroption) {
		...
		if (zSliderEl.getAttr('min', '') !== '') option.min = zSliderEl.getAttr('min').toInt();
		if (zSliderEl.getAttr('max', '') !== '') option.max = zSliderEl.getAttr('max').toInt();
		if (zSliderEl.getAttr('step', '') !== '') option.step = zSliderEl.getAttr('step').toInt();
		...
		// fix option indicator
		if (option.indicatorAsSelectList) {
		...
		var zSliderIndicatorWrapEl = false;
		// xem coi co can su dung toi indicator ko?
		if (option.indicator && option.step > 0) {
			zSliderIndicatorWrapEl = z('<div class="indicators">').appendTo(zSliderWrapEl);
			for (var i = option.min; i <= option.max; i += option.step) {
				zjs('<span>').addClass('num-' + i).html(i + '').appendTo(zSliderIndicatorWrapEl).on('click', (function () {
					var j = i;
					return function (event) {
						event.preventDefault();
						setSliderValue(zSliderEl, j);
					}
				})());
			}
			zSliderIndicatorWrapEl.child().item(0).addClass('num-first');
		}
		...
		if (option.indicatorAsSelectList && option.step > 0 && option.checkMobileDevice()) {
			...
			zjs('<label class="select-label">').setAttr('for', selectElId).appendTo(zSliderSelectWrapEl);
			for (var i = option.min; i <= option.max; i += option.step) {
				zjs('<option>').setAttr('value', i).html('' + i).appendTo(zSliderSelectEl);
			}
			zSliderSelectEl.on('change', function () {
				setSliderValue(zSliderEl, zSliderSelectEl.getValue());
			});
		}
		...
		function moveButton(left) {
			...
			// active cai thang indicator luon
			if (zSliderIndicatorWrapEl && zSliderIndicatorWrapEl.count()) {
				zSliderIndicatorWrapEl.find('span').removeClass('active');
				zSliderIndicatorWrapEl.find('.num-' + sliderValue).addClass('active');
			}
		...
		setSliderValue = function (element, value) {
			...
			// active cai thang indicator luon
			var zSliderIndicatorWrapEl = zSliderWrapEl.find('.indicators');
			if (zSliderIndicatorWrapEl.count()) {
				zSliderIndicatorWrapEl.find('span').removeClass('active');
				zSliderIndicatorWrapEl.find('.num-' + sliderValue).addClass('active');
			}
