...
time:!1,hour:!0,hourmin:0,hourmax:23,hourstep:1,minute:!0,minutemin:0,minutemax:59,minutestep:1,second:!0,secondmin:0,secondmax:59,secondstep:1,button:!0,language:r,firstOfWeek:v[r].firstOfWeek,calendarTitleFormat:v[r].calendarTitleFormat,format:v[r].format,href:"",selectweek:!1,inputClass:"",disabledDate:!1,calendarChangeMonthYear:!0,calendarYearFieldUseInput:!1}});var G=function(p,m){var c=zjs(p),b=c.getData("zmoduleuidatepickeroption");if(b)c.setData("zmoduleuidatepickeroption",zjs.extend(b,m));
else{var b=zjs.clone(zjs.moduleUiDatepickerOption),d=c.getAttr("data-option","");zjs.isString(d)&&""!=d.trim()&&(b=zjs.extend(b,d.jsonDecode()));c.removeAttr("data-option");"undefined"!=typeof m&&(b=zjs.extend(b,m));d=c.getAttr("data-href","");""!=d&&(b.href=d);zjs.isString(b.disabledDate)&&(b.disabledDate=b.disabledDate.split(/\s*,\s*/));zjs.isArray(b.disabledDate)&&(b.disabledDate=function(){var a=b.disabledDate,c=[],e=!1,d=moment().format("YYYY-MM-DD"),g=moment().add(1,"day").format("YYYY-MM-DD"),
h=moment().add(-1,"day").format("YYYY-MM-DD");zjs.foreach(a,function(a){a=a.trim();a=a.replace("today",d).replace("tomorrow",g).replace("yesterday",h).replace("to","<<").replace("before","<").replace("after",">");var b="";0<=a.indexOf("<<")&&a.match(/(\d\d\d\d-\d\d?-\d\d?)\s*<<\s*(\d\d\d\d-\d\d?-\d\d?)/)&&(a=RegExp.$1+"<<",b=RegExp.$2);e=moment(a,"YYYY-MM-DD");if(e.isValid()){var f="=";0<=a.indexOf("<<")?f="<<":0<=a.indexOf("<")?f="<":0<=a.indexOf(">")&&(f=">");"<<"==f&&(e.add(-1,"day"),b=moment(b,
"YYYY-MM-DD").add(1,"day").format("YYYY-MM-DD"));c.push({comparison:f,date:e.format("YYYY-MM-DD"),date2:b})}});return 0>=c.length?!1:function(a){var b=!1,e="";zjs.foreach(c,function(c){"="==c.comparison?e="isSame":"<"==c.comparison?e="isBefore":">"==c.comparison?e="isAfter":"<<"==c.comparison&&(e="isBetween");if(moment(a,"YYYY-MM-DD")[e](c.date,c.date2))return b=!0,!1});return b}}());zjs.isFunction(b.disabledDate)||(b.disabledDate=!1);c.setData("zmoduleuidatepickeroption",b);var k=zjs('<div class="zui-datepicker"><div class="zui-datepicker-input-wrap"><input type="text" class="zui-contextual-button zui-datepicker-input" /></div><div class="zui-datepicker-button-wrap"><a class="zui-contextual-button zui-datepicker-button"></a></div><div class="zui-contextual-panel-wrap zui-datepicker-panel-wrap zui-hide"><div class="zui-datepicker-panel-inner"><div class="zui-datepicker-panel"></div><div class="zui-datepicker-month-change"></div><a class="zui-datepicker-nav zui-datepicker-nav-prev" data-count="-1"><i></i></a><a class="zui-datepicker-nav zui-datepicker-nav-next" data-count="1"><i></i></a><div class="zui-datepicker-time-panel"><div class="zui-datepicker-time-inner"><div class="timeunit hour"><label>hour</label><span></span><input /></div><div class="timeunit minute"><label>minute</label><span></span><input /></div><div class="timeunit second"><label>second</label><span></span><input /></div></div></div></div></div></div>').insertAfter(c);
c.setData("zmoduleuidatepickerwrapel",k);var q=c.getValue(""),d=""==q?moment():moment(q,"YYYY-MM-DD HH:mm:ss");b.hour||d.hour(0);b.minute||d.minute(0);b.second||d.second(0);var r=c.getAttr("placeholder","");d.locale(b.language);var z=k.find(".zui-datepicker-panel-wrap"),t=k.find(".zui-datepicker-panel"),g=k.find(".zui-datepicker-time-panel");b.full&&z.addClass("zui-full");if(b.time){z.on("click",function(a){a.preventDefault();a.stop()});if(b.hour){var u=g.find(".hour"),D=u.find("span");u.find("label").setInnerHTML(v[b.language].timesliderLabel.h);
u.find("input").setValue(d.hour()).makeUiSlider({min:b.hourmin,max:b.hourmax,step:b.hourstep,referer:D}).on("ui:slider:change",function(a){var f=c.getData("zmoduleuidatepickerselectdatetime").hour(a.getData().value);k.find(".zui-button-label").setInnerHTML(f.format(b.format));"setvalue"!=a.getData().via&&(a=f.format("YYYY-MM-DD HH:mm:ss"),c.setAttr("value",a).setValue(a),c.trigger("ui:datepicker:change",{value:a}))})}else g.find(".hour").remove();if(b.minute){var n=g.find(".minute"),D=n.find("span");
...
if(b.second){var w=g.find(".second"),g=w.find("span");w.find("label").setInnerHTML(v[b.language].timesliderLabel.s);w.find("input").setValue(d.second()).makeUiSlider({min:b.secondmin,max:b.secondmax,step:b.secondstep,referer:g}).on("ui:slider:change",function(a){var f=c.getData("zmoduleuidatepickerselectdatetime").second(a.getData().value);k.find(".zui-button-label").setInnerHTML(f.format(b.format));"setvalue"!=a.getData().via&&(a=f.format("YYYY-MM-DD HH:mm:ss"),c.setAttr("value",a).setValue(a),c.trigger("ui:datepicker:change",
{value:a}))})}else g.find(".second").remove();b.hour&&d.hour(u.find("input").getValue());b.minute&&d.minute(n.find("input").getValue());b.second&&d.second(w.find("input").getValue())}else g.remove();g=moment(d);c.setData("zmoduleuidatepickercalendardatetime",g);c.setData("zmoduleuidatepickerselectdatetime",moment(d));u=d.format(b.time?"YYYY-MM-DD HH:mm:ss":"YYYY-MM-DD");if(""!=q||""==r)b.disabledDate&&b.disabledDate(d.format("YYYY-MM-DD"))||(c.setAttr("value",u).setValue(u),c.trigger("ui:datepicker:change",
{value:u}));var n=z.find(".zui-datepicker-month-change"),A=!1,y=!1;if(b.calendarChangeMonthYear){w=moment();w.locale(b.language);var E=function(a){var c="",e=moment();e.locale(b.language);for(var d=0;12>d;d++)c+='<option value="'+d+'" '+(a==d?"selected":"")+" >"+e.month(d).format(b.full?"MMMM":"MMM")+"</option>";return c},g='<select class="zui-calendar-input-month">'+E(g.month()),g=g+"</select>";if(b.calendarYearFieldUseInput)g+='<input class="zui-calendar-input-year" value="'+w.format("YYYY")+'" />';
else var F=function(a){for(var c="",e=a-40;e<=a+20;e++)c+='<option value="'+e+'" '+(e==a?"selected":"")+" >"+(e==a-40?e+" ("+v[b.language].orbefore+")":e==a+20?e+" ("+v[b.language].orafter+")":e)+"</option>";return c},g=g+'<select class="zui-calendar-input-year">'+F(w.year()),g=g+"</select>";n.setInnerHTML(g);A=n.find(".zui-calendar-input-month");y=n.find(".zui-calendar-input-year");k.on("click",".zui-calendar-input-month, .zui-calendar-input-year",function(a){h&&h.hasClass("isfocus")&&(B=!0);a.preventDefault();
