...
zjs.require('tinymce', function(){
	...
	zjs.extendCore({
		moduleEditorOption: {
			css: zjs.requireOption.root+zjs.requireOption.cssFolder+'z.module.editor.reset.css',
			customcss: '',
			baseurl: '',
			converturls: true,
	...
	var zeditorparentclass = 'zeditor-parent',
		zeditorplaceholderclass = 'zeditor-placeholder',
	...
	var makeEditor = function(element, useroption){
		...
		// get ra placeholder
		var placeholderText = zEl.getAttr('placeholder', ''),
			placeholderEl = false;
		...
		var editorOption = {
			...
			theme: function(editor, target){
				// make iframe
				var editorParentEl = zjs('<div style="width:'+mceWidth+'px" id="'+mceParentId+'" class="'+zeditorparentclass + (option.displayastextinput?' inputtext':'') + '"><div class="'+zeditorplaceholderclass+'"></div><div id="'+mceIframeContainerId+'" class="'+zeditoriframecontainerclass+'"></div></div>').insertAfter(target);
				
				placeholderEl = editorParentEl.find('.'+zeditorplaceholderclass);
									...
									if(zjs.isObject(zjs.moduleEditorButtons[button])){
										if('init' in zjs.moduleEditorButtons[button]){
											if(zjs.isFunction(zjs.moduleEditorButtons[button].init)){
												...
												// zEl => o day tuc la cai thang <textarea> do
											}
										}
									}
								}
		...
		// bind event de biet ma bat tat cai placeholder
		var handlerPlaceholder = function(){
			var b = editor.getBody();if(!b)return;
			if(zjs(b).getInnerText()=='')placeholderEl.html(placeholderText);
			else placeholderEl.html('');
			// nhan tien set content luon
			zEl.html(editor.getContent());
		};
		editor.onChange.add(handlerPlaceholder);
		editor.onSetContent.add(handlerPlaceholder);
		editor.onPaste.add(handlerPlaceholder);
		editor.onKeyUp.add(handlerPlaceholder);
		// first trigger
		(function(){
			handlerPlaceholder();
			var d = editor.getDoc();
			if(d)zjs(d).on('load', handlerPlaceholder);
		}).delay(1500);
		handlerPlaceholder.delay(3000);
		
		// 1 cai nho nhung quan trong
		// khi click vao cai thang editor thi phai trigger click vao parent window document
		// (muc dich la de hanler may cai panel - click vao document thi hide)
