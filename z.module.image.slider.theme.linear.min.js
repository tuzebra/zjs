...
L=0,X=e.find(".image-view");e.find(".image-view-border");var g=e.find(".image-view-wrap"),za=e.find(".image-title h4"),Wa=e.find(".image-description"),ja=e.find(".image-linkcover-wrap"),h=-1,q=-1,Na=-1,Aa="",ca="",ka="",M="",m=1,A=-1,C=-1,l=a.width,p=a.height,la=p,Xa=1;if(a.fullscreenWidth||a.fullWidth||a.usePercentWidth)l=a.fullscreenWidth?P.width():R.width();a.fullHeight&&(p=R.height());0>=l&&(l=g.width());0>=l&&(l=R.width());0>=l&&(l=P.width());0>=p&&(p=g.height());0>=p&&(p=parseInt(R.height()));
if(0>=p)try{var u=g.getCss();if(u){var Oa=u.match(/(^|[^a-z-])height:\s*(\d+)\s*px/);f.isArray(Oa)&&(p=parseInt(Oa[2]))}}catch(b){}0>=p&&(p=376);la=p;a.fullHeight||X.height(p);a.fullscreenWidth||a.fullWidth||a.usePercentWidth||3>=a.transition&&X.width(l);g.setStyle("position","absolute");""!==a.imageViewClass&&X.addClass(a.imageViewClass);var Pa=g.item(0,!0),D=0,r=-1,S="cubic-bezier(0, 0, 1, 1)",H=!1,da=function(){H&&g.setStyle("transition","transform "+a.transitionTime+"ms "+S)},ma=function(){H&&
...
da()}f.foreach(d,function(b,c){if(!a.popupOnly&&(""!=b.src||a.contentSlide)){if(a.navThumb){var k=a.navThumbTemplate;f.isFunction(k)&&(k=k(c));k=k.format({index:c,src:b.src,title:b.title,description:b.description});k=f(k).addClass("nav-image").setStyle("background-image","url("+b.src+")").setAttr("data-index",c).appendTo(G).on(a.hoverToSlide?"hover":"click",function(a,b){var c=f(b).getAttr("data-index").toInt();v(c)});b.title&&k.find(".title").setInnerHTML(b.title);L=k.getStyle("margin-right",!0);
if(isNaN(L)||!L)L=a.navImagesElMargin}a.navDot&&(k=a.navDotTemplate,f.isFunction(k)&&(k=k(c)),k=k.format({index:c,src:b.src,title:b.title,description:b.description}),f(k).addClass("nav-dot").setAttr("data-index",c).appendTo(ya).click(function(a,b){var c=f(b).getAttr("data-index").toInt();v(c)}));k=f('<div class="image-hold"></div>');k.appendTo(g);k.addClass("image"+c);k.setData("image-index",c);""!==a.imageHoldClass&&k.addClass(a.imageHoldClass);var e=f('<div class="image-item"></div>');k.append(e);
b.customEl&&e.append(b.customEl);var h=k.width(),h=Va?b.srcpopup||b.srclarge||b.src:250>h?b.src:b.srclarge||b.src;if(a.contentSlide){var n=e.find("[data-lazy-src], [data-lazysrc], [data-srclazy], [srclazy]");n.count()&&(b.lazyEl=n);a.lazyloadImage&&!b.lazyEl&&(n=e.find("[popupsrc], [srcpopup], [data-popupsrc], [data-srcpopup]"),n.count()&&(b.lazyEl=n))}else a.lazyloadImage?(b.srclazy&&e.setStyle("background-image","url("+h+")"),b.lazyEl=e):e.setStyle("background-image","url("+h+")");a.lazyloadImage&&
...
function(a){v(a.getData().index)});e.on("slider:change",function(a){Y.find(".image-hold").removeClass("main-slider-active");var b=Y.find(".image-hold.image"+a.getData().index).addClass("main-slider-active"),d=parseInt(Y.find(".image-view-wrap").getData("real-pos")),e=Y.width(),f=b.left(),b=b.width();(0>d+f||d+f+b>e)&&Y.slideTo(a.getData().index)})}a.linkTitle&&za.hover(function(){e.addClass("linktitlehover")},function(){e.removeClass("linktitlehover")});a.linkCover?ja.hover(function(){e.addClass("linkcoverhover")},
function(){e.removeClass("linkcoverhover")}):ja.remove();e.setStyle("overflow","visible");e.removeClass("slider-not-ready");var Ca=0,v=function(b,c,k,ha,t){k=k||!1;ha=ha||!1;t=t||!1;if(B){b=parseInt(b);var n=1;200<=a.transition&&(n=parseInt(w.width()/l+.1));isNaN(n)&&(n=1);201===a.transition&&1<n&&b+n>=d.length&&(b=d.length-n);T.isRunning()&&T.stop();var N=1;c&&"undefined"!=typeof c.direction&&"prev"==c.direction&&(N=-1);b=Da(b,N);if(2!=K&&"function"==typeof a.allowChange&&a.allowChange(b)){if(q=
h,h=b,Na=y.indexOf(b),t||k||h!=q){d[b]&&(Aa=d[b].srclarge||d[b].src,ca=d[b].title,M=d[b].link,ka=d[b].description,M=d[b].link);f.moduleTransition&&e.find(".image"+q+" ."+a.customTransitionClass).stopTransition().unsetTransition();c=c||{};c=f.extend({index:h,oldindex:q,image:Aa,title:ca,description:ka,link:M},c);g.find(".image"+h).setStyle("z-index",Xa++);k=k||!1;switch(a.transition){case 1:H?(g.find(".image"+h).setStyle("opacity",1),1<d.length&&g.find(".image"+q).setStyle("opacity",0)):Za.stop().run();
...
202==a.transition||203==a.transition||204==a.transition||205==a.transition)&&0==h&&c>O||(2==a.transition||201==a.transition||202==a.transition||203==a.transition||204==a.transition||205==a.transition)&&h==Ga-1&&c<O||3==a.transition&&0==h&&b>O||3==a.transition&&h==Ga-1&&b<O)d=3;x(Fa+na/d)}}},onDrop:function(a,c,d,e){2==K&&(K=0,a=D<Fa?h+1:h-1,a=Math.min(Math.max(a,0),Ga-1),da(),a!=h?v(a,{direction:a>h?"next":"prev"}):x(-l*a))}});var Ta=!1,oa=function(){Ta?(v(h,{},!0,!0,!0),Q.trigger("resize")):Ta=!0},
Da=function(a,c){c=c||1;if(y.length==d.length||-1<y.indexOf(a))return a;var b;if(0<c){a>=d.length&&(a=0);for(b=0;b<y.length;b++)if(y[b]>a)return y[b];return Da(0,c)}if(0>c){0>a&&(a=d.length-1);for(b=y.length-1;0<=b;b--)if(y[b]<a)return y[b];return Da(d.length-1,c)}},ea=function(a){!(a=a||!1)&&xa||!w||(U=f("<div></div>").addClass("imageslider-raw"),U.insertAfter(w),U.setInnerHTML(va),e.addClass("slider-disabled"),B=!1,a&&(wa=!0),a&&(xa=!1),e.trigger("slider:disable"),U.find("[data-lazy-src], [data-lazysrc], [data-srclazy], [srclazy]").eachElement(function(a){a=
z(a);var b=a.getAttr("data-lazy-src","");""==b&&(b=a.getAttr("data-lazysrc",""));""==b&&(b=a.getAttr("data-srclazy",""));""==b&&(b=a.getAttr("srclazy",""));""==b&&(b=a.getAttr("srclazy",""));""==b&&(b=a.getAttr("data-lazy-src",""));""!=b&&a.setAttr("src",b)}))},fa=function(a){if((a=a||!1)||!wa)U&&U.remove(),e.removeClass("slider-disabled"),B=!0,a&&(wa=!1),a&&(xa=!0),e.trigger("slider:enable")};if(0<a.autoDisableWhenWidthLessThan||0<a.autoDisableWhenWidthLargerThan||0<a.autoDisableWhenWindowWidthLessThan||
0<a.autoDisableWhenWindowWidthLargerThan){var $a=function(){var b=a.fullscreenWidth?P.width():R.width(),c=Q.width();0<a.autoDisableWhenWidthLessThan&&(b<=a.autoDisableWhenWidthLessThan&&!0===B?ea():b>a.autoDisableWhenWidthLessThan&&!1===B&&fa());0<a.autoDisableWhenWidthLargerThan&&(b>=a.autoDisableWhenWidthLargerThan&&!0===B?ea():b<a.autoDisableWhenWidthLargerThan&&!1===B&&fa());0<a.autoDisableWhenWindowWidthLessThan&&(c<=a.autoDisableWhenWindowWidthLessThan&&!0===B?ea():c>a.autoDisableWhenWindowWidthLessThan&&
!1===B&&fa());0<a.autoDisableWhenWindowWidthLargerThan&&(c>=a.autoDisableWhenWindowWidthLargerThan&&!0===B?ea():c<a.autoDisableWhenWindowWidthLargerThan&&!1===B&&fa())};Q.on("resize",function(){$a.delay(500)}).trigger("resize")}if(a.fullscreenWidth||a.fullWidth||a.fullHeight){(a.fullscreenWidth||a.fullWidth)&&e.addClass("fullwidth");a.fullHeight&&e.addClass("fullheight");var pa=function(){var b=a.fullscreenWidth?P.width():R.width();if(B){if(2===a.transition||203===a.transition)a.sequent||(r=r*b/l),
a.usePercentWidth||g.find(".image-hold").eachElement(function(c,d){var e=y.indexOf(d);if(!(0>e)){var k=f(c);a.sequent?k.left(b*e):k.left(k.left()*b/l)}}),x(r);l=b;var c=p;0<a.scaleHeightWhenWidthLessThan&&(l<=a.scaleHeightWhenWidthLessThan?(a.fullHeight&&e.removeClass("fullheight"),p=la*l/a.scaleHeightWhenWidthLessThan):a.fullHeight?(e.addClass("fullheight"),p="",la=R.height()):p=la,g.height(p),X.height(p));3===a.transition&&(a.sequent||(r=r*p/c),g.find(".image-hold").eachElement(function(b,d){var e=
...
closethenremove:!0,pagecover:!0,clickout:a.popupHideOnClickOut,pressEsc:a.popupAllowKeyboard,fade:!0,fadeTime:a.popupAnimateTime,animate:a.popupAnimate,animateTime:a.popupAnimateTime,animateName:a.popupAnimateName,scrollPopup:a.popupScrollPopup}).on("ui:popup:hide",function(){ga=!1});else k.fadeIn({time:a.popupAnimateTime}),a.popupAnimate&&g.addClass("zui-popup hide-before-animate "+a.popupAnimateName).setStyle("animation-duration",a.animateTime+"ms");n.on("slider:ready",function(){Ia();c?g.popupShow():
...
slidePlay:sa,slidePause:Ia,slidePopupShow:ia,slidePopupHide:Ja,slideRefresh:oa,slideDisable:function(){ea(!0)},slideEnable:function(){fa(!0)},slideFilter:function(b){var c;for(c=0;c<d.length;c++){var e=!0;if(f.isFunction(b)){var g=b(d[c]);"undefined"!==typeof g&&(e=g)}d[c].isShown=e}Ma();b=!1;a.navDot&&(b=ya.find(".nav-dot"),b.count()||(b=!1));for(c=0;c<d.length;c++)d[c].isShown?(d[c].el.removeClass("slide-hide"),b.item(c).show()):(d[c].el.addClass("slide-hide"),b.item(c).hide());oa()},getSliderInfo:function(){return{images:d,
current:{index:h,image:Aa,title:ca,description:ka,link:M}}}}});f(document).on("keydown",function(F){if(37==F.keyCode()||39==F.keyCode()||27==F.keyCode()){var d=f(".inpopup-imageslider.allow-keyboard");0>=d.count()||(37==F.keyCode()&&d.slidePrev(),39==F.keyCode()&&d.slideNext(),27==F.keyCode()&&d.getData("parent_SliderEl").slidePopupHide())}});"required"in f&&f.required("image.slider.theme.linear")});